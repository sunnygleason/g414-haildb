...
public class Database {
    ...
    public Database(DatabaseConfiguration c) {
        ...
        Util.assertSuccess(HailDB.ib_cfg_set("data_file_path",
                c.getDatafilePath()));
        Util.assertSuccess(HailDB.ib_cfg_set("data_home_dir",
                c.getDataHomeDir()));
        ...
        Util.assertSuccess(HailDB.ib_cfg_set("lru_block_access_recency",
                c.getLruBlockAccessRecency()));
        Util.assertSuccess(HailDB.ib_cfg_set("lru_old_blocks_pct",
                c.getLruOldBlocksPct()));
        Util.assertSuccess(HailDB.ib_cfg_set("max_dirty_pages_pct",
                c.getMaxDirtyPagesPct()));
        Util.assertSuccess(HailDB.ib_cfg_set("max_purge_lag",
                c.getMaxPurgeLagSeconds()));
        Util.assertSuccess(HailDB.ib_cfg_set("open_files",
                c.getOpenFilesLimit()));
        Util.assertSuccess(HailDB.ib_cfg_set("autoextend_increment",
                c.getAutoextendIncrementSizePages()));
        Util.assertSuccess(HailDB.ib_cfg_set("io_capacity",
                c.getIoCapacityIOPS()));
        Util.assertSuccess(HailDB.ib_cfg_set("sync_spin_loops",
                c.getSyncSpinLoops()));
        Util.assertSuccess(HailDB.ib_cfg_set("lock_wait_timeout",
                c.getLockWaitTimeoutSeconds()));
    ...
    public void createDatabase(String databaseName) {
        Util.assertSchemaOperationSuccess(HailDB
    ...
    public void createTable(TableDef tableDef, TableType type, int pageSize) {
        ...
        for (ColumnDef def : tableDef.getColumnDefs().values()) {
            ...
            if (!def.getType().equals(ColumnType.BLOB)) {
                Util.assertSuccess(HailDB.ib_table_schema_add_col(schema
        ...
        for (Map.Entry<String, IndexDef> entry : tableDef.getIndexDefs()
            ...
            Util.assertSuccess(HailDB.ib_table_schema_add_index(
                    schema.getValue(), entry.getKey(), index));
            ...
            for (ColumnDef col : part.getColumns()) {
                ...
                Util.assertSuccess(HailDB.ib_index_schema_add_col(
                        index.getValue(), col.getName(), prefixLenOverride));
            }

            if (part.isClustered()) {
                Util.assertSuccess(HailDB.ib_index_schema_set_clustered(index
                        .getValue()));
            } else if (part.isUnique()) {
                Util.assertSuccess(HailDB.ib_index_schema_set_unique(index
                        .getValue()));
            }
        }
        ...
        try {
            ...
            Util.assertSuccess(HailDB.ib_table_create(trx.getTrx(),
                    schema.getValue(), tableId));
            trx.commit();
    ...
    private void shutdown(boolean fast) {
        int flag = fast ? HailDB.ib_shutdown_t.IB_SHUTDOWN_NO_BUFPOOL_FLUSH
                : HailDB.ib_shutdown_t.IB_SHUTDOWN_NORMAL;
