...
public class Database {
	...
	public Database(DatabaseConfiguration c) {
		...
		Util.assertSuccess(HailDB.ib_cfg_set("data_file_path", c
				.getDatafilePath()));
		Util.assertSuccess(HailDB.ib_cfg_set("data_home_dir", c
				.getDataHomeDir()));
		Util.assertSuccess(HailDB.ib_cfg_set("log_group_home_dir", c
		...
		Util.assertSuccess(HailDB.ib_cfg_set("lru_old_blocks_pct", c
				.getLruOldBlocksPct()));
		Util.assertSuccess(HailDB.ib_cfg_set("max_dirty_pages_pct", c
				.getMaxDirtyPagesPct()));
		Util.assertSuccess(HailDB.ib_cfg_set("max_purge_lag", c
				.getMaxPurgeLagSeconds()));
		Util.assertSuccess(HailDB.ib_cfg_set("open_files", c
				.getOpenFilesLimit()));
	...
	public void createTable(TableDef tableDef, TableType type, int pageSize) {
		...
		for (Map.Entry<String, IndexDef> entry : tableDef.getIndexDefs()
			...
			Util.assertSuccess(HailDB.ib_table_schema_add_index(schema
			...
			for (ColumnDef col : part.getColumns()) {
				...
				Util.assertSuccess(HailDB.ib_index_schema_add_col(index
		...
		try {
			...
			Util.assertSuccess(HailDB.ib_table_create(trx.getTrx(), schema
					.getValue(), tableId));
			trx.commit();
